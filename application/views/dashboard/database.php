...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    <div class="breadcrumb-title pe-3">ฐานข้อมูลก๊าซเรือนกระจก</div>
    <div class="ps-3">
        ...
        <div class="row">
            ...
            <div style="position: relative;">
                    <div style="position: absolute;top:80px;width:100%;text-align:center;">
                        <h1 class="text-white" style="text-shadow: 
        3px 3px 5px rgba(0, 0, 0, 0.3), 
        -2px -2px 3px rgba(255, 255, 255, 0.4), 
        4px 4px 15px rgba(0, 0, 0, 0.5); ">ฐานข้อมูลก๊าซเรือนกระจก</h1>
                    </div>
                ...
                <?php foreach($rsYear as $item){?>
                    ...
                    <?php if($fa == 'bx-edit' || in_array($item->year_value, $is_data) ){?>
                        <div class="d-grid mb-3"><a href="<?=base_url()?><?=$uri?>/<?=$item->year_value?>/<?=$id?>" class="btn btn-<?=in_array($item->year_value, $is_data)?'theme':'secondary'?>"><i class="bx <?=$fa?>"></i> <?=$item->year_name?> <?=getDataSet($rsMember['member_dataset'], $item->year_value)?></a></div>
