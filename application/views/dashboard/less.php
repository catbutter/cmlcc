            ...
            <div class="col-md-6 offset-md-3">
                <div style="position: relative;">
                    <div style="position: absolute;top:80px;width:100%;text-align:center;">
                        <h1 class="text-white" style="text-shadow: 3px 3px 5px rgba(0, 0, 0, 0.3), -2px -2px 3px rgba(255, 255, 255, 0.4), 4px 4px 15px rgba(0, 0, 0, 0.5); ">กิจกรรมการลด</h1>
                    </div>
                ...
                <?php foreach($rsYear as $item){?>
                    ...
                    <?php if($fa == 'bx-edit' || in_array($item->year_value, $is_data) ){?>
                        <div class="d-grid mb-3"><a href="<?=base_url()?><?=$uri?>/<?=$item->year_value?>/<?=$id?>" class="btn btn-<?=in_array($item->year_value, $is_data)?'theme':'secondary'?>"><i class="bx <?=$fa?>"></i> บันทึกข้อมูลกิจกรรมการลด ปี พ.ศ.<?=$item->year_name?></a></div>
